 ...
 */
KISSY.add('sea/dos-provider/0.0.6/index',["node","base","io","json"],function(S ,require, exports, module) {
    ...
    var DosProvider = Base.extend({
        ...
        sendReport:function(type){
            var self=this;
            et = new Date().getTime();
            if(!window.__WPO) return;
            var url='?widgetId='+self.get('config').data.widgetId;
            switch (type){
                case 'normal':

                    __WPO.retCode(url,true,et-st,'接口返回正常');
                    break;
                case 'cache':
                    __WPO.retCode(url,true,et-st,'接口错误，采用缓存数据');
                    break;
                case 'static':
                    __WPO.retCode(url,false,et-st,'接口错误，采用容灾数据');
                    break;
                default :
                    __WPO.retCode(url,false,et-st,'接口错误');
                    break;
            }
